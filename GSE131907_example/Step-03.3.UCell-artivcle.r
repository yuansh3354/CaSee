...
dir(file_path)
file_path = paste0(file_path,'GSE131907_Cancer_article_label_wilcoxon.csv')
...
sce = sce[,which(!is.na(sce$cnv.label))]
...
for(module_name in module_names){
  module[paste0(module_name,'_cnv.label')] = gene_list[,module_name] %>% unique() %>% 
    intersect(DEGs[,1]) %>% as.character() %>% list()
}
sce <- AddModuleScore_UCell(sce, features = module)


paste0(module_names,'_cnv.label', "_UCell")
features = c('EMT_cnv.label_UCell','Energy.metabolism_cnv.label_UCell')
p1 = RidgePlot(sce, features=features[1],cols = c("#E64B35B2",'#698EC3')
               , group.by = "cnv.label") 
p2 = RidgePlot(sce, features=features[2],cols = c("#E64B35B2",'#698EC3')
               , group.by = "cnv.label") 
p3 = DotPlot(sce,features=features,cols = c("lightgrey", "#e67d6b")
             ,group.by = "cnv.label")
x = CombinePlots(plots = list(p1,p2,p3),ncol = 1)
x
topptx(x,file="Module_score_article.pptx",  width = 12,
       height = 9)
