...
dir(file_path)
file_path = paste0(file_path,'GSE131907-Cancer_scale_predict_wilcoxon.csv')
...
for(module_name in module_names){
  module[paste0(module_name,'_Predict')] = gene_list[,module_name] %>% unique() %>% 
           intersect(DEGs[,1]) %>% as.character() %>% list()
}
sce <- AddModuleScore_UCell(sce, features = module)


paste0(module_names,'_Predict', "_UCell")
features = c('EMT_Predict_UCell','Energy.metabolism_Predict_UCell')

p1 = RidgePlot(sce, features=features[1],cols = c("#E64B35B2",'#698EC3')
               , group.by = "scale_predict") 
...
x
topptx(x,file="Module_score_scale_predict.pptx",  width = 12,
       height = 9)
